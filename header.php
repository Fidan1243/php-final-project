...
if ($action != "") {
    $result = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM `news` WHERE `id`=$newsid LIMIT 1"));
    if (!isset($_COOKIE[$newsid])) {
        setcookie($newsid, $action);
        $update = $action . "_count";
        $update_number = $result[$action . "_count"] + 1;
        mysqli_query($conn, "UPDATE `news` SET `$update` = $update_number WHERE `id`=$newsid");
    }
}
        ...
        <header class="header">
            ...
            <div class="header-primary">
                <div class="l-center">
                    <div class="header-side">
                        <div class="engage"><a class="engage-i engage-i_android" target="_blank" href="https://play.google.com/store/apps/details?id=az.start.android.oxu">Android App</a><a class="engage-i engage-i_ios" target="_blank" href="https://itunes.apple.com/app/id634362981">iOS App</a></div>
                        <?php
                        $ru_az = $lang == 1 ? "ru":"az";
                        ...
                        ?>
                    </div><a class="logo logo-oxu" href="../project/">oxu.az</a>
            ...
            <div class="nav-container">
                <div class="l-center">
                    <nav class="nav">
                        <?php
                        $sql = "SELECT * FROM `category` WHERE `lang`=$lang ORDER BY `order`";
                        $result = mysqli_query($conn, $sql);
                        while ($row = mysqli_fetch_assoc($result)) {
                            $current = $page == $row["id"] ? "nav-i_current" : "";
                            echo '<a class="nav-i ' . $current . '" href="../project/?page=' . $row["id"] . '">' . $row["name"] . '</a>';
                        }
                        ?>
                    </nav>
                    <div class="side-menu large-only">
                        <div id="hamburger">
                            <div></div>
                        </div>
                        <div class="side-category">
                            <ul>
                                <?php
                                $sql = "SELECT * FROM `category` WHERE `lang`=$lang AND `order`>9 ORDER BY `order`";
                                $result = mysqli_query($conn, $sql);
                                while ($row = mysqli_fetch_assoc($result)) {
                                    $current = $page == $row["id"] ? "nav-i_current" : "";
                                    echo '<li><a class="nav-i ' . $current . '" href="../project/?page=' . $row["id"] . '">' . $row["name"] . '</a></li>';
                                }
                                ?>
                            </ul>
                        </div>
